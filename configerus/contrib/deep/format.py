...
class ConfigFormatDeepPlugin():
    ...
    def format(self, target, default_source:str):
        ...
        if isinstance(target, Dict):
            return self.format_dict(target, strip_missing)
        elif isinstance(target, List):
            return self.format_list(target, strip_missing)
    ...
    def format_list(self, target: List[Any], strip_missing: bool = False):
        ...
        for index, value in enumerate(target):
            target[index] = self.format(value, default_source)
        return target
    ...
    def format_dict(self, target: Dict[str, Any], strip_missing: bool = False):
        ...
        for key, value in target.items():
            target[key] = self.format(value, default_source)
        return target
