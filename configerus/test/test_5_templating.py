...
config_sources = [
    {
        ...
        "data": {
            "config": {
                ...
                "2": {"1": "first 2.1"},
                "3": {
                    "1": {"1": "first 3.1.1", "2": "first 3.1.2"},
                    "2": {"1": "first 3.2.1", "2": "first 3.2.2"},
                },
            },
            "variables": {
                "one": "first one",
...
class ConfigTemplating(unittest.TestCase):
    ...
    def test_format_passthrough(self):
        ...
        self.assertEqual(
            self.config.format("{{value::myvalue}}", "config"), "myvalue"
        )
    ...
    def test_format_dot_notation(self):
        ...
        self.assertEqual(
            self.config.format("{{3.2.1}}", "config"), "first 3.2.1"
        )
    ...
    def test_format_embedding_and_casting(self):
        ...
        self.assertEqual(
            self.config.format("12345{{1}}67890", "config"),
            "12345first 167890",
        )
        self.assertEqual(
            self.config.format("1 23 45 {{1}}6 78 90", "config"),
            "1 23 45 first 16 78 90",
        )
        """ format replacements in the middle of a string """

        self.assertEqual(
            self.config.format("{{2}}", "config"), {"1": "first 2.1"}
        )
        self.assertEqual(
            self.config.format(" {{2}} ", "config"), " {'1': 'first 2.1'} "
        )
    ...
    def test_format_with_sources(self):
        ...
        self.assertEqual(
            self.config.format("{{variables:two}}", "config"), variables_two
        )
        self.assertEqual(
            self.config.format("{{variables:two}}", "variables"), variables_two
        )
    ...
    def test_format_with_defaults(self):
        ...
        self.assertEqual(
            self.config.format("{{does.not.exist?default1}}", "config"),
            "default1",
        )
        self.assertEqual(
            self.config.format(
                "{{variables:does.not.exist?default2}}", "config"
            ),
            "default2",
        )
        ...
        self.assertEqual(variables_two, "first two")
        self.assertEqual(
            self.config.format("{{variables:two?default}}", "config"),
            variables_two,
        )
    ...
    def test_get_formatted(self):
        ...
        self.assertEqual(
            self.loaded_config.format("{{variables:three}}"), variables_three
        )
        self.assertEqual(
            self.loaded_config.format(" {{variables:three}} "),
            " {} ".format(variables_three),
        )
    ...
    def test_file_templating(self):
        ...
        self.assertEqual(
            replace_json,
            replace_config.format("{{file::{{paths:replace}}/replace.json}}"),
        )
        self.assertEqual(
            "default",
            replace_config.format(
                "{{file::{{paths:replace}}/no_such_file.json?default}}"
            ),
        )
    ...
    def test_crazy_stuff(self):
        ...
        self.assertEqual(
            self.loaded_config.format(" A{{40}} Z"),
            " A{} Z".format(config_fourty),
        )

        self.assertEqual(
            self.loaded_config.format("A{{value::target}}Z"), "AtargetZ"
        )

        self.assertEqual(
            self.loaded_config.format("A{{get::40?default}}Z"),
            "A{}Z".format(config_fourty),
        )

        self.assertEqual(
            self.loaded_config.format("A{{fail?default}}Z"), "AdefaultZ"
        )

        self.assertEqual(
            self.loaded_config.format(
                "A{{get::{{value::40}}?value::custom}}Z"
            ),
            "A{}Z".format(config_fourty),
        )
        ...
        self.assertEqual(
            self.loaded_config.format(
                "A{{value::{{value::{{value::test}}}}?value::custom}}Z"
            ),
            "AtestZ",
        )

        self.assertEqual(
            self.loaded_config.format("A{{get::fail?default}}Z"), "AdefaultZ"
        )

        self.assertEqual(
            self.loaded_config.format(
                "A{{fail?get::fail?get::fail?default}}Z"
            ),
            "AdefaultZ",
        )
    ...
    def test_special_formats(self):
        ...
        self.assertIsNone(
            self.loaded_config.format("{{fail?get::fail?special::None}}")
        )
